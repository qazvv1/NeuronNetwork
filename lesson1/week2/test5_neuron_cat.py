...
def propagate(w, b, X, Y):
    ...
    A = sigmoid(np.dot(w.T, X) + b).reshape(1, m)                      # compute activation
    cost = -(np.dot(Y, np.log(A).reshape(m, 1)) + np.dot((1 - Y), np.log(1 - A).reshape(m, 1))) / m                           # compute cost
    ### END CODE HERE ###
...
def predict(w, b, X):
    ...
    A = sigmoid((np.dot(w.T, X) + b).reshape(1,m) )
